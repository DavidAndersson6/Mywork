...
bouncingImages.imgHeight = 200;
bouncingImages.imgSrc = 'barney.jpg';
...
function loadBouncingImages() {
   if (dyn && !bouncingImages.isLoaded) {
      ...
      for (var layerLoop = 0; layerLoop < bouncingImages.imgCount; layerLoop++) {
         ...
         bouncingImages.posX[layerLoop] = Math.floor(Math.random() * (600) + 500) + getDocScrollLeft();
         bouncingImages.posY[layerLoop] = Math.floor(Math.random() * (450) + 20) + getDocScrollTop();
...
function moveBouncingImages() {
   for (var layerLoop = 0; layerLoop < bouncingImages.imgCount; layerLoop++) {
      if (bouncingImages.dirX[layerLoop] == 'left') {
         if (bouncingImages.posX[layerLoop] > 500 - bouncingImages.speedX[layerLoop]) bouncingImages.posX[layerLoop] -= bouncingImages.speedX[layerLoop];
         else {
            bouncingImages.dirX[layerLoop] = 'right';
            bouncingImages.posX[layerLoop] = 500;
            }
         }
      else if (bouncingImages.dirX[layerLoop] == 'right') {
         if (bouncingImages.posX[layerLoop] + bouncingImages.imgWidth < 1100 - bouncingImages.speedX[layerLoop]) bouncingImages.posX[layerLoop] += bouncingImages.speedX[layerLoop];
         else {
            bouncingImages.dirX[layerLoop] = 'left';
            bouncingImages.posX[layerLoop] = 1100 - bouncingImages.imgWidth;
            }
         }
      if (bouncingImages.dirY[layerLoop] == 'up') {
         ...
         if (bouncingImages.posY[layerLoop] + bouncingImages.imgHeight < 650 - bouncingImages.speedY[layerLoop]) bouncingImages.posY[layerLoop] += bouncingImages.speedY[layerLoop];
         else {
            bouncingImages.dirY[layerLoop] = 'up';
            bouncingImages.posY[layerLoop] = 650 - bouncingImages.imgHeight;
            }
         }
      }
